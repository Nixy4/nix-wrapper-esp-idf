...
bool Ili9881c::Init(DsiBus& bus, DsiDisplayConfig& config)
{
    vendor_cfg_ = {
    ...
    return DsiDisplay::Init(bus, config, esp_lcd_new_panel_ili9881c, nullptr, &vendor_cfg_,
                            [this, &bus, &config]()
                            {
                                vendor_cfg_.mipi_config.dsi_bus = bus.GetHandle();
                                vendor_cfg_.mipi_config.dpi_config = &config.dpi_config;
                            });
}
