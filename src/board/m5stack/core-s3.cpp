...
LvglPortConfig lvgl_port_config(5,                                    // task_priority
                                8192,                                 // task_stack
                                1,                                    // task_affinity
...
LvglDisplayConfig lvgl_display_config(320 * 240,               // buffer_size
                                      true,                    // double_buffer
                                      0,                       // trans_size
                                      320,                     // hres
                                      240,                     // vres
...
std::function<esp_err_t()> spk_codec_new_func = []() -> esp_err_t
{
    aw88298_codec_cfg_t aw88298_cfg = {
        ...
        .hw_gain = {.pa_voltage = 5.0f, .codec_dac_voltage = 3.3f, .pa_gain = 1.f},
    };
...
bool M5StackCoreS3::InitBus(bool i2c1, bool spi, bool i2s)
{
    if (i2c1)
    {
        if (!i2c_bus1.Init(i2c_bus1_config))
            return false;
    }
    ...
    if (i2s)
    {
        if (!i2s_bus.Init(bus_config))
            return false;
        if (!i2s_bus.ConfigureTxChannel(tx_config))
            return false;
        if (!i2s_bus.ConfigureRxChannel(rx_config))
            return false;
    }
    return true;
}
...
bool M5StackCoreS3::InitDevice(bool power, bool audio, bool display, bool touch)
{
    ...
    {
        // AXP2101
        if (!axp2101.Init(i2c_bus1, axp2101_config))
            return false;

        uint8_t data = 0x00;
        if (!axp2101.ReadReg8(0x90, data, -1))
            return false;
        data |= 0b10110100;
        ...
        {
            if (!axp2101.WriteReg8(reg, value, -1))
                return false;
        }
        axp2101.GetLogger().Info("Configured successfully");

        // AW9523
        if (!aw9523.Init(i2c_bus1, aw9523_config))
            return false;

        std::tuple<uint8_t, uint8_t> aw_cmds[] = {
            {0x02, 0b00000111}, {0x03, 0b10001111}, {0x04, 0b00011000}, {0x05, 0b00001100},
            {0x11, 0b00010000}, {0x12, 0b11111111}, {0x13, 0b11111111},
        };
    ...
    if (touch)
    {
        if (!ft5x06.Init(i2c_bus1, ft5x06_config, esp_lcd_touch_new_i2c_ft5x06))
            return false;
    }
...
bool M5StackCoreS3::Init()
{
    if (initialized_)
        return true;
    if (!InitBus(true, true, true))
        return false;
    if (!InitDevice(true, true, true, true))
        return false;
    if (!InitMiddleware(true))
        return false;
...
I2cBus& M5StackCoreS3::GetI2cBus1() { return i2c_bus1; }

SpiBus& M5StackCoreS3::GetSpiBus() { return spi_bus; }

I2sBus& M5StackCoreS3::GetI2sBus() { return i2s_bus; }

Axp2101& M5StackCoreS3::GetPowerManagement() { return axp2101; }

Aw9523& M5StackCoreS3::GetGpioExpander() { return aw9523; }

LvglPort& M5StackCoreS3::GetLvglPort() { return lvgl_port; }

AudioCodec& M5StackCoreS3::GetAudioCodec() { return audio_codec; }
